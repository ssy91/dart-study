void main() {
  ...
  // Dart에서는 사용자가 실수로 null 값을 이용하지 않게 하기 위해서
  // null safety 라는 기본 기능을 제공해줍니다.
  ...
  // 그래서 String 타입도 들어가고, null 도 들어갈 수 있게 하려면 ㄷ음과 같은 기능을 이용해주어야 합니다.
  ...
  // 에러를 전달하고 있어서, 컴퓨터에게 nico 가 null이 아닐 때 해당 코드를 실행할 수 있게 해주어야 함.
  if (nico != null) {
    nico.length;
  }
  ...
  // nul을 사용하게 해주기 위해서는 타입의 뒤에 ? 를 붙여주어야 한다. 예) String? nico;
}

// main 에서 string 이라는 String 타입을 받아와서, 해당 길이가 0인기 확인하는 함수
