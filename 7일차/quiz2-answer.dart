...
Future main() async {
  ...
  await for (HttpRequest request in server) {
    ...
    // contry 에 있는 나라에 맞는 수도를 출력하는 프로그램을 만들어보자!
    // 사용자의 요청이 /country 로 보낸 것인지 확인부터 해보자!
    // request.uri.path 즉 사용자가 요청한 경로에 /country 라는 문자열이 있는지 확인
    // coutains : 문자열.contains(확인할 내용) 문자열에 contains 이후에 작성된
    // 확인할 내용이 있는지에 대한 true, false 값을 반환하는 기능
      ...
      // 이제 country 변수에서 사용자가 입력한 나라가 있으면
      // 그 나라의 수도를 출력하는 부분을 만들어보자
      if (country.containsKey(input)) {
        // print(country[input]);
        var sudo = country[input]; //sudo 라는 변수에 결과값을 넣어서 보기 편하게 변경
    ...
    // 이 응답은 이렇게 끝내겠다 라고 하는 "reponse 를 닫는 기능"을 작성해줘야 합니다.
