...
void create(Map database, HttpRequest request) async {
  var bodyDate = await utf8.decoder.bind(request).join();
  // 생성 코드 작성
  ...
  var mapData = jsonDecode(bodyDate) as Map;
  ...
  // 근데 추가할 때 같은 데이터를 추가하면 어떻게 되는거지?
  ...
  printAndSendHttpResponse(database, request, bodyDate);
}
    ...
    if (request.uri.path.contains('/user') == true) {
      ...
      try {
        switch (request.method) {
          // 메서드들에 대한 CASE 작성
          case "GET":
            // CRUD 에서 READ 를 담당할 때 GET 사용
            read(database, request);
            break;
          case "PUT":
            // CRUD 에서 UPDATE 를 담달할 때 PUT 사용
            update(database, request);
            break;
          case "POST":
            // CRUD 에서 CREATE 를 담달할 때 POST 사용
            create(database, request);
            break;
          case "DELETE":
            // CRUD 에서 DELETE 를 담달할 때 DELETE 사용
